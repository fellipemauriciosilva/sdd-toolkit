...
Você está executando como `github-copilot`. Use `execute/runInTerminal` para executar comandos e `execute/getTerminalOutput` para ler a saída. Registre `last_runtime: github-copilot` no session-state ao finalizar.
...
Use `execute/runInTerminal` para rodar e `execute/getTerminalOutput` para ler a saída:
