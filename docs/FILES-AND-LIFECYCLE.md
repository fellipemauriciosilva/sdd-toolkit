...
| Workspace user | specs, `task.md`, packs, estado, resultados e evidências | fora do repositório consumidor; apenas o `sdd-orchestrator` registra a orquestração |
...
geradas. O orquestrador cria packs em `contexts/` e os agentes recebem somente o
...
flowchart TB
    ...
    P --> A[Agente]
    A --> O[AGENT_RESULT]
    O --> V[orquestrador valida e registra]
    V --> S
    V --> E[events.ndjson]
    V --> R
    V --> X[evidence]
