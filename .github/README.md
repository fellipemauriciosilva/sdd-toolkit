# Context Engineering & SDD Kit — v3.1
...
> Maturidade atual: **N4 (v3.1)** — agent com loop, quality gates com integridade verificável, 3 checkpoints humanos, evals, versionamento, observabilidade e estrutura runtime-agnostic com source único e compilação para múltiplos runtimes.
...
## Modelo v3.x — Source Único + Compilador

A partir da v3.0, o kit abandonou a manutenção de arquivos separados por runtime. Cada agente tem **um único arquivo fonte** em `agents/` com seções marcadas por runtime:

```markdown
<!-- @all -->     conteúdo comum a todos os runtimes
<!-- @claude -->  conteúdo exclusivo Claude Code
<!-- @copilot --> conteúdo exclusivo GitHub Copilot
<!-- @end -->     fecha a seção
```

O compilador embutido em `install.ps1`/`install.sh` filtra as seções, injeta o frontmatter correto e grava em `dist/`:

| Saída | Destino final | Frontmatter |
|-------|--------------|-------------|
| `dist/copilot/*.agent.md` | `<PROJECT>/.github/agents/` | `mode: agent`, `tools:`, `model:` |
| `dist/claude/*.md` | `<PROJECT>/.claude/agents/` e `~/.claude/agents/` | `name:`, `description:` |

Recursos deployáveis (gates config, templates de spec, skills, instructions, decisions-log) ficam em `templates/`. O `.github/` do kit contém apenas metadados do repositório.
...
## Estrutura do Kit (sdd-toolkit/)

O kit tem separação clara entre fonte, recursos deployáveis e artefatos gerados:

```
sdd-toolkit/
├── agents/                          # ← SOURCE ÚNICO: 18 agentes com seções @all/@claude/@copilot
│   ├── sdd-bootstrap.md
│   ├── sdd-analyze-demand.md
│   ├── sdd-implement-spec.md
│   └── …                           # (18 arquivos .md)
│
├── templates/                       # ← RECURSOS DEPLOYÁVEIS (copiados para projetos pelo install)
│   ├── sdd-gates.config.md          # Configuração de gates padrão
│   ├── session-state.md             # Template de estado do pipeline
│   ├── decisions-log.md             # Template do decision log
│   ├── instructions/                # 5 instruction files (java, spring-boot, tests, security, pull-request)
│   ├── skills/                      # 20+ skills por stack (doc-reader, pdf-reader, java, python…)
│   └── specs/
│       └── types/                   # 4 templates por tipo (feature, bugfix, refactor, migration)
│
├── dist/                            # ← ARTEFATOS GERADOS (nunca editar manualmente)
│   ├── copilot/                     # *.agent.md com frontmatter Copilot (mode: agent)
│   └── claude/                      # *.md com frontmatter Claude Code (name:)
│
├── .github/                         # ← METADADOS DO KIT (não são deployados)
│   ├── README.md, ROADMAP.md, sdd-kit-status.md
│   ├── evals/                       # 21 casos de eval para 7 agentes principais
│   └── images/                      # Diagramas PNG finais
│
├── install.ps1                      # Compilador + instalador (Windows/PowerShell)
├── install.sh                       # Compilador + instalador (Linux/Mac)
└── workspace/                       # specs e session-states por projeto (gerado pelo install)
    └── <project>/specs/<TICKET>/
```

> **Como funciona:** `install.ps1 <PROJECT> -Runtime all` compila `agents/*.md` → `dist/`, depois copia `dist/copilot/` → `<PROJECT>/.github/agents/` e `dist/claude/` → `<PROJECT>/.claude/agents/`. Recursos de `templates/` são copiados para o projeto conforme necessário.
...
│
├── .claude/
│   └── agents/            # Agentes para Claude Code — gerados de dist/claude/ pelo install
│       ├── sdd-bootstrap.md
│       ├── sdd-analyze-demand.md
│       └── …              # (18 agentes)
│
├── .github/
│   ├── sdd.config.md                # Referência ao kit e ao projeto (sdd_kit: + project:)
│   ├── AGENTS.md                    # Comportamento obrigatório do agente
│   ├── copilot-instructions.md      # Contexto principal do projeto
│   ├── sdd-gates.config.md          # Políticas de gate do projeto (copiado de templates/)
│   ├── PIPELINE-STATUS.md           # Dashboard de gates (gerado por sdd-workspace-sync)
│   │
│   ├── agents/                      # Agentes para GitHub Copilot — gerados de dist/copilot/ pelo install
│   │   ├── sdd-bootstrap.agent.md
│   │   ├── sdd-analyze-demand.agent.md
│   │   └── …              # (18 agentes)
│   │
│   ├── instructions/                # Regras técnicas (copiadas de templates/instructions/ pelo install)
...
│       │   ├── module-map.md
│       │   └── decisions-log.md     # Log append-only de decisões entre demandas
...
│
~/.claude/agents/
└── sdd-bootstrap.md                 # Orquestrador global (Claude Code — deployado pelo install)
```

> **Dois runtimes, uma fonte:** `agents/*.md` no kit é o source único. O compilador gera os artefatos: `dist/copilot/*.agent.md` (frontmatter `mode: agent`) para GitHub Copilot e `dist/claude/*.md` (frontmatter `name:`) para Claude Code. Os artefatos são então copiados para o projeto — nunca editados diretamente.
...
O template está em `templates/session-state.md` no kit (copiado para o projeto pelo `install.ps1`/`install.sh`).
...
O template de criação inicial está em `templates/decisions-log.md` no kit — criado pelo `sdd-install-sdd-kit` no projeto.
...
Templates em `templates/specs/types/` no kit; referenciados pelos agentes como `{sdd_kit}/templates/specs/types/`.
...
Versões no frontmatter de `agents/*.md`: fluxo principal e setup em `2.5.0`; especializados em `2.3.0`. O bootstrap registra `agente@versão` no Agent History e avisa quando a versão muda entre execuções.
...
### Via script (recomendado — v3.1)

O kit inclui um compilador/instalador que compila os agentes fonte e deploya para o projeto:

```powershell
# Windows (PowerShell)
cd sdd-toolkit
powershell install.ps1 <CAMINHO_ABSOLUTO_DO_PROJETO> -Runtime all
```
...
O script:
1. Cria `workspace/<project>/specs/` no kit (fonte de verdade das demandas)
2. Cria `<PROJECT>/.github/sdd.config.md` com referência ao kit
3. Cria estrutura `.github/` no projeto
4. Copia `templates/sdd-gates.config.md` → `<PROJECT>/.github/sdd-gates.config.md` (se não existir)
5. Compila `agents/*.md` → `dist/copilot/` e `dist/claude/`
6. Deploya `dist/copilot/` → `<PROJECT>/.github/agents/`
7. Deploya `dist/claude/` → `<PROJECT>/.claude/agents/`
8. Deploya `dist/claude/sdd-bootstrap.md` → `~/.claude/agents/` (global)

### Após a instalação

Preencha o contexto do projeto em qualquer runtime:
...
### Agente de instalação (alternativa sem script)

Se não tiver acesso ao kit diretamente, o agente `sdd-install-sdd-kit` pode configurar a estrutura básica:

```
@sdd-install-sdd-kit    # Copilot
/sdd-install-sdd-kit    # Claude Code
```
...
O kit inclui 5 instruction files em `templates/instructions/`, aplicadas automaticamente pelo Copilot conforme o arquivo editado:

| Instruction | Escopo |
|-------------|--------|
| `java.instructions.md` | Boas práticas Java gerais |
| `spring-boot.instructions.md` | Padrões Spring Boot (controllers, services, config) |
| `tests.instructions.md` | Regras de testes unitários e integração |
| `security.instructions.md` | OWASP, validação de entrada, secrets |
| `pull-request.instructions.md` | Padrões de PR e mensagens de commit |

Copiados para `<PROJECT>/.github/instructions/` pelo `install.ps1`/`install.sh`. O Claude Code lê via `CLAUDE.md` usando `@path` ou por instrução do agente.
...
Consulte [sdd-kit-status.md](sdd-kit-status.md) para a lista de projetos com o kit instalado, versão e runtime ativo. Para uma visão consolidada dos pipelines em execução, consulte `PIPELINE-STATUS.md` dentro de cada projeto ou execute `/sdd-workspace-sync`.
